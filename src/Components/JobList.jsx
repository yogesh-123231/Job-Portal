...
const categories = ["View all", "Design", "Development", "Marketing", "Finance", "Customer Service"];
const types = ["All Types", "Full-time", "Part-time", "Remote", "Intern"];
const locations = ["All Locations", "Remote", "Mumbai", "Pune", "Delhi", "Bangalore", "Hyderabad", "Chennai", "Gurgaon"];
const experiences = ["All Levels", "Entry Level", "Mid Level", "Senior Level"];
...
const JobList = () => {
  ...
  const applyFilters = (jobsData) => {
    return jobsData.filter((job) => {
      const matchCategory = category === "View all" || job.category === category;
      const matchType = type === "All Types" || (Array.isArray(job.type) && job.type.includes(type));
      const matchLocation = location === "All Locations" || job.location === location;
      const matchExperience = experience === "All Levels" || job.experience === experience;
  ...
  }, [category, type, location, experience]);
      ...
      <section className="bg-[#141419] text-white px-6 md:px-20 py-16 font-script overflow-x-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <select value={experience} onChange={(e) => setExperience(e.target.value)} className="bg-[#1f1f2b] px-4 py-2 rounded text-white">
            {experiences.map((exp, i) => <option key={i} value={exp}>{exp}</option>)}
          </select>
                ...
                <div className="flex items-start gap-3 md:gap-4">
                  ...
                  <div className="ml-1 md:ml-5">
                    ...
                    <div className="flex gap-1 md:gap-2 my-2 flex-wrap">
                      {Array.isArray(job.type) && job.type.map((t, i) => (
                        <span
                          key={i}
